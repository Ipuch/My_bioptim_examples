...
def set_p(biorbd_model: biorbd.Model, value: MX):
    """
...
def prepare_ocp(
    ...
    bound_p = Bounds(0.1, 10, interpolation=InterpolationType.CONSTANT)
    initial_p = InitialGuess(1)

    parameters = ParameterList()
    parameters.add(
        "p",  # The name of the parameter
        set_p,
        initial_p,  # The initial guess
        bound_p,  # The bounds
        size=1,  # The number of elements this particular parameter vector has
        scaling=np.array([1]),
        extra_value=dict(value=1)  # You can define as many extra arguments as you want
    )

    dynamics = DynamicsList()
    dynamics = Dynamics(DynamicsFcn.TORQUE_DRIVEN, with_contact=True, dynamic_function=custom_dynamic, parameters=set_p)
