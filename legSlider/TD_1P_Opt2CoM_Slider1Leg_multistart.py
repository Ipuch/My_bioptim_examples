...
if __name__ == "__main__":
    ...
    t = []
    n = 40
    initial_guess_CoM_list=np.linspace(-2, 2, n)
    CoM = np.zeros((n, 6))
    Cost = np.zeros((n, 1))
    for i in range(len(initial_guess_CoM_list)):
        opti_CoM = True
        cur_ini = initial_guess_CoM_list[i]
        ocp = prepare_ocp(
            ...
            max_CoM=[0, 1, .8],
            min_CoM=[0, -1, -.8],
            initial_guess_CoM=[0, 0, cur_ini],  # try .79, -.79, 0, 0.1
            segment_idx=[1, 2],
        )
        # --- Solve the program --- #
        sol = ocp.solve()
        ...
        # t.append(sol.real_time_to_optimize)
        # sol.ocp.g[0][0]['constraint'].name
        # sol.constraints[:]
        # x = sol.print()
        # val = []
        # val_weighted = []
        ...
        # --- Get the results --- #
        if opti_CoM:
            Cost[i] = sol.cost.toarray()[0]
            CoM[i, :] = sol.parameters["CoMs"].T
            print(f'{" Results ":*^30}')
    ...
    plt.plot(initial_guess_CoM_list, CoM[:, 2],'o',ms=1)
    plt.grid()
    plt.show()
    # plt.axes(aspect='equal')
    plt.plot(x=[-2, 2], y=[2, 2], ls='-')